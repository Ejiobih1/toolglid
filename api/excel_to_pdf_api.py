"""
Excel to PDF API - Standalone endpoint for Excel to PDF conversion
...
# ============== Configuration ==============
UPLOAD_DIR = Path(tempfile.gettempdir()) / "excel_to_pdf_uploads"
OUTPUT_DIR = Path(tempfile.gettempdir()) / "excel_to_pdf_outputs"
...
# ============== Router ==============
router = APIRouter(prefix="/excel-to-pdf", tags=["Excel to PDF"])
...
async def convert_with_libreoffice(input_file: Path, output_dir: Path) -> Path:
    ...
    print(f"[Excel→PDF] stdout: {stdout_text}")
    print(f"[Excel→PDF] stderr: {stderr_text}")
    print(f"[Excel→PDF] return code: {process.returncode}")
    ...
    if output_file.exists():
        print(f"[Excel→PDF] Success: {output_file}")
        return output_file

    # Check for any PDF in output directory
    pdf_files = list(output_dir.glob("*.pdf"))
    if pdf_files:
        print(f"[Excel→PDF] Found PDF: {pdf_files[0]}")
        return pdf_files[0]
...
async def health():
    """Health check"""
    return {"service": "excel-to-pdf", "status": "ok"}
...
async def convert(file: UploadFile = File(...)):
    ...
    try:
        ...
        print(f"[Excel→PDF] Processing: {filename} ({size} bytes)")
